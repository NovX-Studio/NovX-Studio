      ...
      <ContainerScroll
        titleComponent={
          <div className="text-center px-6">
            <p className="text-[10px] font-semibold tracking-[0.25em] text-foreground/25 uppercase mb-4">Proyectos</p>
            <h2 className="font-serif text-[clamp(2rem,5vw,3.5rem)] font-normal text-foreground leading-tight mb-4">
              Lo que <em className="text-foreground/40">construimos.</em>
            </h2>
            <p className="text-foreground/30 text-sm max-w-md mx-auto mb-6">
              Proyectos reales, código limpio, resultados medibles.
            </p>
            <Link href="/contacto">
              <button className="bg-primary hover:bg-primary/90 text-primary-foreground font-semibold text-sm px-6 py-2.5 rounded-full transition-all duration-200 hover:scale-105">
                Empezar el tuyo
              </button>
            </Link>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 h-full p-4 overflow-auto">
          {proyectos.map((p, i) => (
            <motion.div
              ...
              className="relative rounded-xl overflow-hidden border border-foreground/5 group cursor-default bg-bg-base"
            >
              <div className="relative h-36 overflow-hidden">
                ...
                <span className="absolute top-3 left-3 text-[10px] font-semibold tracking-widest text-foreground/60 uppercase bg-background/80 px-2 py-1 rounded-full border border-foreground/10">
                  {p.tag}
                </span>
              </div>
              <div className="p-4">
                <h3 className="font-serif text-base font-normal text-foreground mb-1">{p.title}</h3>
                <p className="text-foreground/35 text-xs leading-relaxed mb-3">{p.desc}</p>
                <div className="flex flex-wrap gap-1.5">
                  {p.tech.map((t) => (
                    <span key={t} className="text-[10px] font-mono text-foreground/25 bg-foreground/5 px-2 py-0.5 rounded-full border border-foreground/5">{t}</span>
                  ))}
                </div>
              </div>
            </motion.div>
