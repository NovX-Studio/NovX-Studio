    ...
    <div className="min-h-screen pt-24 pb-20 px-6">
      <div className="max-w-6xl mx-auto">
        <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} className="mb-20">
          <p className="text-[10px] font-semibold tracking-[0.25em] text-black/25 uppercase mb-4">Lo que hacemos</p>
          <h1 className="font-serif text-[clamp(2.5rem,6vw,4.5rem)] font-normal text-black leading-tight mb-5">Servicios</h1>
          <p className="text-black/35 text-lg leading-relaxed max-w-xl">Construimos con las tecnologías correctas, sin sobre-ingenierizar. Cada proyecto tratado con la misma obsesión por el detalle.</p>
        </motion.div>
        ...
        <motion.div variants={containerVariants} initial="hidden" animate="visible" className="space-y-4">
          {services.map((service) => (
            <motion.div
              ...
              className="relative p-8 md:p-10 rounded-2xl border bg-bg-card border-black/5 hover:border-black/10 transition-all duration-250"
            >
              <div className="grid grid-cols-1 md:grid-cols-[auto_1fr_auto] gap-6 md:gap-10 items-start">
                <div className="flex md:flex-col items-center md:items-start gap-3">
                  <span className="text-sm font-mono text-black/20">{service.number}</span>
                  <span className="text-3xl text-black/20">{service.icon}</span>
                </div>
                <div>
                  <h2 className="font-serif text-2xl font-normal mb-3 text-black">{service.title}</h2>
                  <p className="text-sm leading-relaxed mb-5 text-black/35">{service.description}</p>
                  <ul className="flex flex-wrap gap-2">
                    {service.features.map((f) => (
                      <li key={f} className="text-xs font-medium px-3 py-1 rounded-full border border-black/8 text-black/30 bg-black/3">{f}</li>
                    ))}
                  </ul>
                </div>
                <div className="md:self-center">
                  <Link href="/contacto">
                    <button className="text-xs font-semibold px-5 py-2.5 rounded-full border border-black/15 text-black/50 hover:border-black/40 hover:text-black transition-all duration-200 whitespace-nowrap hover:scale-105">
                      {service.cta}
                    </button>
                  </Link>
                </div>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }} className="mt-20 text-center">
          <p className="text-black/25 mb-5 text-base">¿No encontrás lo que buscás? <span className="text-black/50">Hablemos igual.</span></p>
          <Link href="/contacto">
